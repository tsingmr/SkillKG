...
def negative_sampling(pos_samples, num_entity, negative_rate):
    size_of_batch = len(pos_samples)    # 获取当前batch的大小
    num_to_generate = size_of_batch * negative_rate # 计算需要生成的负样本数量
    neg_samples = np.tile(pos_samples, (negative_rate, 1))  # 将正样本复制negative_rate次作为负样本的初始值
    labels = np.zeros(size_of_batch * (negative_rate + 1), dtype=np.float32)    # 定义标签，正样本标签为1，负样本标签为0
    labels[: size_of_batch] = 1

    values = np.random.choice(num_entity, size=num_to_generate) # 生成随机实体id用于采样负样本
    choices = np.random.uniform(size=num_to_generate)   # 随机均匀采样正样本的头尾实体
    subj = choices > 0.5
    obj = choices <= 0.5
    neg_samples[subj, 0] = values[subj] # 将采样到的实体id替换负样本的头尾实体
